...
def count_T(H,P,p_c,N_users):
    T_comm,T_pri=[],[]
    for k in range(N_users):
        T_private_k=1
        for i in range(N_users):
            T_private_k+=abs_square_of_Hk_mul_Pi(H,P,k,i)
        T_c=T_private_k+abs_square_of_Hk_mul_Pi(H,p_c,k,0)
        T_comm.append(T_c)
...
def count_I(T_pri,H,P,N_users):
    ...
    return I_comm,I_pri
def count_SINR(I_comm,I_pri,N_users,p_c,P,H):
    SINR_comm,SINR_pri=[],[]
    for k in range(N_users):
        SINR_comm.append(abs_square_of_Hk_mul_Pi(H,p_c,k,0)/I_comm[k])
        SINR_pri.append(abs_square_of_Hk_mul_Pi(H,P,k,k)/I_pri[k])
    return SINR_comm,SINR_pri

def count_beta(alpha_comm,alpha_pri,N_users,p_c,P,T_comm,T_pri,H):
    beta_comm,beta_pri=[],[]
    for k in range(N_users):
        beta_comm_k=np.sqrt(1+alpha_comm[k])*Hk_mul_Pi(H,p_c,k,0)/T_comm[k]
...
def count_first_part_of_update_P_comm(lambda_i,beta_comm,H,mu,N_users,Nt):
    return count_mc(lambda_i,beta_comm,H,N_users)+mu*np.eye(Nt)

def count_mc(lambda_i,beta_comm,H,N_users):
    summ=0
    for k in range(N_users):
        summ+=lambda_i[k]*(abs(beta_comm[k])**2)*square_of_Hk(H, k)
    return summ

def count_second_part_of_update_P_comm(alpha_comm,beta_comm,lambda_i,H,N_users):
    summ=0
    for k in range(N_users):
        summ+=np.sqrt(1+alpha_comm[k])*beta_comm[k]*lambda_i[k]* H[:, k][:, np.newaxis]
    return summ

def update_P_comm(alpha_comm,beta_comm,lambda_i,mu,H,N_users,Nt):
    part_1=count_first_part_of_update_P_comm(lambda_i,beta_comm,H,mu,N_users,Nt)
    part_2=count_second_part_of_update_P_comm(alpha_comm,beta_comm,lambda_i,H,N_users)
    return np.linalg.inv(part_1)@part_2

def count_mp(lambda_i,beta_pri,beta_comm,H,N_users):
    summ=0
    for k in range(N_users):
        summ+=(abs(beta_pri[k])**2+lambda_i[k]*(abs(beta_comm[k])**2))*square_of_Hk(H,k)
    return summ

def count_first_part_of_update_P_pri(lambda_i,beta_pri,beta_comm,H,mu,N_users,Nt):
    return count_mp(lambda_i,beta_pri,beta_comm,H,N_users)+mu*np.eye(Nt)

def count_second_part_of_update_P_pri(alpha_pri,beta_pri,H,k):
    summ=np.sqrt(1+alpha_pri[k])*beta_pri[k]*H[:, k][:, np.newaxis]
    return summ

def update_P_pri(alpha_pri,beta_comm,beta_pri,lambda_i,H,N_users,Nt,mu):
    P=[]
    part_1 = count_first_part_of_update_P_pri(lambda_i, beta_pri, beta_comm, H, mu, N_users, Nt)
...
def count_g_comm(H,p_c,alpha_comm,beta_comm,N_users,T_comm):
    g=[]
    for k in range(N_users):
        g_part_1=np.log2(1+alpha_comm[k])-alpha_comm[k]
        g_part_2=2*np.sqrt(1+alpha_comm[k])*np.real(np.conj(beta_comm[k].T)*Hk_mul_Pi(H,p_c,k,0))
        g_part_3=(abs(beta_comm[k])**2)*T_comm[k]
...
def update_lambda(g_comm,rou,lambda_i,N_users):
    lambda_new=[0 for i in range(len(lambda_i))]
    y=count_y(g_comm)
    index = np.argmin(g_comm)
    summ=0
    sum_abs=0
    if y>=0:
        for k in range(N_users):
            lambda_new[k]=((y+rou)/(g_comm[k]+rou))*lambda_i[k]
            summ+=lambda_i[k]-lambda_new[k]
            sum_abs+=abs(summ)
    else:
        for k in range(N_users):
            lambda_new[k]=(rou-y)/(g_comm[k]-2*y+rou)*lambda_i[k]
            summ+=lambda_i[k]-lambda_new[k]
            sum_abs+=abs(summ)
    lambda_new[index]=lambda_i[index]+summ
    return lambda_new,sum_abs
...
def count_avg_alpha_beta_one(H,P,p_c,N_users):
    T_comm, T_pri = count_T(H, P, p_c, N_users)
    I_comm, I_pri = count_I(T_pri, H, P, N_users)
    alpha_comm, alpha_pri =count_SINR(I_comm, I_pri, N_users, p_c, P, H)
    beta_comm, beta_pri = count_beta(alpha_comm, alpha_pri, N_users, p_c, P, T_comm, T_pri, H)
...
def count_m_n_r_one(H_reals,numbers_of_H_real,P,p_c,N_users,Nt):
    m_p_s,m_c_s=[np.zeros((Nt,Nt)) for k in range(N_users)],[np.zeros((Nt,Nt)) for k in range(N_users)]
    n_p_s,n_c_s=[np.zeros((Nt,1)) for k in range(N_users)],[np.zeros((Nt,1)) for k in range(N_users)]
    r_c_s,r_p_s=[0 for k in range(N_users)],[0 for k in range(N_users)]
    for i in range(numbers_of_H_real):
        H_real=H_reals[i]
        alpha_comm, beta_comm,alpha_pri,beta_pri=count_avg_alpha_beta_one(H_real, P, p_c, N_users)
        beta_pri_square=[abs(beta_pri[k])**2 for k in range(N_users)]
        beta_comm_square=[abs(beta_comm[k])**2 for k in range(N_users)]
        ...
        m_c=[beta_comm_square[k]*square_of_H[k] for k in range(N_users)]
        n_c=[np.sqrt(1+alpha_comm[k])*beta_comm[k]*H_real[:, k][:, np.newaxis] for k in range(N_users)]
        r_c=[np.log2(1+alpha_comm[k])-beta_comm_square[k]-alpha_comm[k] for k in range(N_users)]
        r_p=[np.log2(1+alpha_pri[k])-beta_pri_square[k]-alpha_pri[k] for k in range(N_users)]
        m_p_s=[m_p_s[k]+m_p[k] for k in range(N_users)]
        m_c_s=[m_c_s[k]+m_c[k] for k in range(N_users)]
        n_p_s=[n_p_s[k]+n_p[k] for k in range(N_users)]
        n_c_s=[n_c_s[k]+n_c[k] for k in range(N_users)]
        r_c_s=[r_c_s[k]+r_c[k] for k in range(N_users)]
        r_p_s=[r_p_s[k]+r_p[k] for k in range(N_users)]
    m_p_avg = [m_p_s[k]/numbers_of_H_real for k in range(N_users)]
    m_c_avg = [m_c_s[k]/numbers_of_H_real for k in range(N_users)]
    n_p_avg= [n_p_s[k]/numbers_of_H_real for k in range(N_users)]
    n_c_avg = [n_c_s[k]/numbers_of_H_real for k in range(N_users)]
    r_c_avg= [r_c_s[k]/numbers_of_H_real for k in range(N_users)]
    r_p_avg = [r_p_s[k] /numbers_of_H_real for k in range(N_users)]
    #print("why",tt*10000)
    return m_p_avg,m_c_avg,n_p_avg,n_c_avg,r_c_avg,r_p_avg

def count_alpha_beta_avg(H_reals,numbers_of_H_real,P,p_c,N_users):
    alpha_comm_s, beta_comm_s,alpha_pri_s,beta_pri_s=[],[],[],[]
    beta_comm_square,beta_pri_square=[],[]
    for i in range(numbers_of_H_real):
        H_real=H_reals[i]
        alpha_comm, beta_comm,alpha_pri,beta_pri=count_avg_alpha_beta_one(H_real, P, p_c, N_users)
        alpha_comm_s.append(alpha_comm)
        beta_comm_s.append(beta_comm)
        alpha_pri_s.append(alpha_pri)
        beta_pri_s.append(beta_pri)
        beta_comm_square.append([abs(beta_comm[i])**2 for i in range (N_users)])
        beta_pri_square.append([abs(beta_pri[i])**2for i in range (N_users)])
    alpha_comm_avg=count_avg_of_alpha_beta(alpha_comm_s, numbers_of_H_real, N_users)
    beta_comm_square_avg=count_avg_of_alpha_beta(beta_comm_square, numbers_of_H_real, N_users)
    ...
    return alpha_comm_s,beta_comm_s,alpha_pri_s,beta_pri_s,alpha_comm_avg,beta_comm_square_avg,alpha_pri_avg,beta_pri_square_avg
...
def update_P_p(lambda_i,m_p,n_p,m_c,N_users,Nt,mu):
    P=[]
    part1=mu*np.eye(Nt)+1j*np.zeros((Nt,Nt))
    for i in range(N_users):
        part1 += lambda_i[i] * m_c[i] + m_p[i]
    for k in range(N_users):
        P_k=np.linalg.inv(part1)@n_p[k]
        P.append(P_k)
    return np.concatenate(P,axis=1)

def update_P_c(lambda_i,m_c,n_c,N_users,Nt,mu):
    part1=mu*np.eye(Nt)+1j*np.zeros((Nt,Nt))
    part2=np.zeros_like(n_c[0])
    for i in range(N_users):
        part1+=lambda_i[i]*m_c[i]
        part2+=lambda_i[i]*n_c[i]
    P_c=np.linalg.inv(part1)@part2
    return P_c
...
def count_T_avg(H_reals,numbers_of_H_real, P, p_c, N_users):
    T_comm_sum,T_pri_sum=[],[]
    for i in range(numbers_of_H_real):
        T_comm, T_pri = count_T(H_reals[i], P, p_c, N_users)
        T_comm_sum.append(T_comm)
        T_pri_sum.append(T_pri)
    T_pri_avg=count_avg_of_alpha_beta(T_pri_sum,numbers_of_H_real,N_users)
    T_com_avg=count_avg_of_alpha_beta(T_comm_sum,numbers_of_H_real,N_users)
    return T_com_avg, T_pri_avg

def count_T_I_avg(H_reals,numbers_of_H_real, P, p_c, N_users):
    T_comm_sum, T_pri_sum = [], []
    I_comm_sum, I_pri_sum = [], []
    for i in range(numbers_of_H_real):
        T_comm, T_pri = count_T(H_reals[i], P, p_c, N_users)
        I_comm, I_pri = count_I(T_pri, H_reals[i], P, N_users)
        T_comm_sum.append(T_comm)
        T_pri_sum.append(T_pri)
        I_comm_sum.append(I_comm)
        I_pri_sum.append(I_pri)
    
    T_pri_avg=count_avg_of_alpha_beta(T_pri_sum,numbers_of_H_real,N_users)
    T_com_avg=count_avg_of_alpha_beta(T_comm_sum,numbers_of_H_real,N_users)
    I_pri_avg=count_avg_of_alpha_beta(I_pri_sum,numbers_of_H_real,N_users)
    I_com_avg=count_avg_of_alpha_beta(I_comm_sum,numbers_of_H_real,N_users)
    return T_pri_avg,T_com_avg,I_pri_avg,I_com_avg

def quad_form(P,m,i,k):
    sum=((np.conj(P[:, i][:, np.newaxis].T)@m[k])@P[:, i][:, np.newaxis])[0][0]
    return sum

def count_beta_mul_T_avg(m_p,m_c, P, p_c, N_users):
    T_comm_sum, T_pri_sum = [], []
    for k in range(N_users):
        T_private_k = 0
        T_c=0
        for i in range(N_users):
            ...
            T_c+= quad_form(P,m_c,i,k)
        T_c = T_c +  quad_form(p_c,m_c,0,k)
        T_comm_sum.append(T_c)
        T_pri_sum.append(T_private_k)
    return T_comm_sum, T_pri_sum


def order(H_estimate,H_reals,Nt,N_users,rho,tolerance,Pt,alpha,tolerance_inner,maxcount):
    p_c,P=P_total_init(Pt,H_estimate,N_users,alpha)
    flag=1
    obj_past=0
    count=0
    ...
    while(flag):
        count_inner = 0
        mu = 10 / Pt
        m_p, m_c, n_p, n_c, r_c, r_p=count_m_n_r_one(H_reals,numbers_of_H_real,P,p_c,N_users,Nt)
        while (count_inner <= maxcount):
            p_c=update_P_c(lambda_i,m_c,n_c,N_users,Nt,mu)
            P=update_P_p(lambda_i,m_p,n_p,m_c,N_users,Nt,mu)
            beta_mul_T_comm, beta_mul_T_pri=count_beta_mul_T_avg(m_p, m_c, P, p_c, N_users)
            g_comm=count_g_c(r_c,n_c,p_c,beta_mul_T_comm,N_users)
            lambda_new,summ=update_lambda(g_comm,rho,lambda_i,N_users)
        ...
        if count >= 1000:
            break
    rates_pri,rates_comm=count_avg_rate(H_reals, numbers_of_H_real, P, p_c, N_users)
    obj = sum(rates_pri) + min(rates_comm)
    T2=time.time()
    return obj,T2-T1

def count_avg_rate(H_reals, numbers_of_H_real, P, p_c, N_users):
    rates_comm_sum, rates_pri_sum = [], []
    for i in range(numbers_of_H_real):
        T_comm, T_pri = count_T(H_reals[i], P, p_c, N_users)
        I_comm, I_pri = count_I(T_pri, H_reals[i], P, N_users)
        rates_comm = count_rate(T_comm, I_comm, N_users)
        rates_pri = count_rate(T_pri, I_pri, N_users)
        rates_comm_sum.append(rates_comm)
        rates_pri_sum.append(rates_pri)

    rates_pri_avg = count_avg_of_alpha_beta(rates_pri_sum, numbers_of_H_real, N_users)
    rates_comm_avg = count_avg_of_alpha_beta(rates_comm_sum, numbers_of_H_real, N_users)
    return rates_pri_avg,rates_comm_avg
