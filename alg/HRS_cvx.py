...
def Init_P_g(H,Pt,N_users,groups,alpha,groups_weight):
    P=[]
    power = Pt * alpha[-1]
    ...
    if N_users==4:
        for group_order in range(len(groups)):
            for i in range(len(groups[group_order])):
                power_pri_1 = power_pri * groups_weight[i]
...
def count_T_MMSE_g(H,P,N_users,groups):
    T=[]
    for k in range(N_users):
        T_k=[]
        T_private_k=1
        for i in range(N_users):
            T_private_k+=abs_square_of_Hk_mul_Pi(H,P,k,1+len(groups)+i)
        for group_order in range(len(groups)):
            if k not in groups[group_order]:
                T_private_k+=abs_square_of_Hk_mul_Pi(H,P,k,1+group_order)
        T_k.append(T_private_k)
        for group_order in range(len(groups)):
            if k  in groups[group_order]:
                T_private_k+=abs_square_of_Hk_mul_Pi(H,P,k,1+group_order)
                T_k.append(T_private_k)
                break
...
def count_I_MMSE_g(T,H,P,N_users,groups):
    I=[]
    for k in range(N_users):
        I_k=T[k][1:len(T[k])+1]
        I_k.append(I_k[-1]-abs_square_of_Hk_mul_Pi(H,P,k,1+len(groups)+k))
        I.append(I_k)
    return I

def count_g_MMSE_g(H,P,T,N_users,groups):
    g=[]
    for k in range(N_users):
        g_k=[]
        ...
        for group_order in range(len(groups)):
            if k  in groups[group_order]:
                g_k.append(count_g_i(Pi_mul_Hk(H, P, k, 1 +group_order),T[k][1]))
        g_k.append(count_g_i(Pi_mul_Hk(H,P,k,1+k+len(groups)),T[k][-1]))
        g.append(g_k)
    return g
...
def count_eplison_cvx_g(T,g,H,P,N_users,groups):
    eplison=[]
    for k in range(N_users):
        eplison_k=[]
        ...
        for group_order in range(len(groups)):
            if k  in groups[group_order]:
                eplison_k.append(count_e_cvx(g[k][1],H,P,k,1+group_order,T[k][1]))
        eplison_k.append(count_e_cvx(g[k][-1],H,P,k,1+len(groups)+k,T[k][-1]))
        eplison.append(eplison_k)
    return eplison

def ksai_comm_constraint(x,ksai,N_users,groups):
    constraints=[]
    x_comm_sum=1
    x_group_sum=[1 for i in range(len(groups))]
    ksai_comm=[]
    ksai_group=[[],[]]
    for k in range(N_users):
        x_comm_sum+=x[k][0]
        ksai_comm.append(ksai[k][0])
        for group_order in range(len(groups)):
            if k in groups[group_order]:
                x_group_sum[group_order] += x[k][1]
                ksai_group[group_order].append(ksai[k][1])
    for k in range(N_users):
        constraints.append(ksai[k][0] <= x_comm_sum)
        for group_order in range(len(groups)):
            if k in groups[group_order]:
                constraints.append(ksai[k][1] <= x_group_sum[group_order])
...
def count_ksai_imp(u,t,phi,f,v,P_opt,N_users,groups):
    ksai=[]
    for k in range(N_users):
        ksai_k=[0 for i in range(3)]
        for i in range(N_users):
            for m_j in range(3):
                ksai_k[m_j]+=quad_form(P_opt,phi,1+len(groups)+i,k,m_j)
        for group_order in range(len(groups)):
            if k not in groups[group_order]:
                for m_j in range(3):
                    ksai_k[m_j]+=quad_form(P_opt,phi,1+group_order,k,m_j)
        for group_order in range(len(groups)):
            if k in groups[group_order]:
                for m_j in range(2):
                    ksai_k[m_j] += quad_form(P_opt, phi, 1 + group_order, k, m_j)
        ...
        for group_order in range(len(groups)):
            if k in groups[group_order]:
                ksai_k[1]=count_ksai(ksai_k[1],t,f,P_opt,v,u,group_order+1,k,1)
        ksai_k[0] = count_ksai(ksai_k[0], t, f, P_opt, v, u, 0, k, 0)
        ksai_k[-1] = count_ksai(ksai_k[-1], t, f, P_opt, v, u, 1+len(groups)+k, k, -1)
        ksai.append(ksai_k)
    return ksai

def optimize_P_x_g(groups,u,t, phi, f, v,Pt,miu_weight_WSR,N_users,Nt,rth):
    P_opt = cvx.Variable((Nt, N_users+len(groups)+1),complex=True)
    x=cvx.Variable((N_users,2))
    ksai=count_ksai_imp(u,t,phi,f,v,P_opt,N_users,groups)
    ksai_tot = count_ksai_tot_g(ksai, x,N_users)
    sum_P=0+0j
    for i in range(N_users+len(groups)+1):
        sum_P+=cvx.quad_form(P_opt[:,i][:, np.newaxis],np.eye(Nt))
    constraint1 = ksai_comm_constraint(x,ksai,N_users,groups)
    constraint2 = cvx.real(sum_P)<= Pt
    constraint3 = []
    for k in range(N_users):
        constraint3.append(ksai_tot[k] <= 1-rth)
    constraint4 = x<= np.zeros((N_users,2))
    constraints=[]
...
def count_v(u,N_users):
    ...
    for k in range(N_users):
        ...
        v.append(v_k)
    return v
def count_one_of_avg(H_real,P,N_users,groups):
    T = count_T_MMSE_g(H_real, P, N_users, groups)
    I = count_I_MMSE_g(T, H_real, P, N_users, groups)
    e = count_eplison_MMSE_g(T, I, N_users)
    g = count_g_MMSE_g(H_real, P, T, N_users, groups)
...
def count_avg(number_of_H_real,H_reals,P,N_users,Nt,number_of_precoder_one_user,groups):
    ...
    for i in range(len(H_reals)):
        H_real=H_reals[i]
        u,t,phi,f,v=count_one_of_avg(H_real, P, N_users,groups)
        for k in range(N_users):
            for j in range(number_of_precoder_one_user):
                u_sum[k][j]+=u[k][j]
                t_sum[k][j]+=t[k][j]
...
def count_avg_rate(number_of_H_real,H_reals,P,N_users,groups):
    rates=[]
...
def order_g(H_estimate,H_reals,group_numbers,miu_weight_WSR,boundary,Pt,N_users,Nt,rth,alphas,groups_weight):
    sum = [0]
    L=[]
    groups=[]
    numbers = 0
    for i in range(len(group_numbers)):
        L=[]
        for j in range(group_numbers[i]):
            L.append(numbers)
            numbers+=1
        groups.append(L)
    P=Init_P_g(H_estimate,Pt,N_users,groups,alphas,groups_weight)
    n = 0
    T1=time.time()
    while True:
        n += 1
        number_of_H_real=len(H_reals)
        u,t, phi, f, v=count_avg(number_of_H_real,H_reals,P,N_users,Nt,3,groups)
        P,x,obj = optimize_P_x_g(groups,u,t, phi, f, v,Pt,miu_weight_WSR,N_users,Nt,rth)
        sum.append(obj)
        if abs(sum[n] - sum[n - 1]) < boundary or n>=1000:
            rates=count_avg_rate(number_of_H_real,H_reals,P,N_users,groups)
            rate_tot=count_rate_tot(rates, x, N_users)
...
def init_H_2(Nt,N_users):
    thetas=[0,8*np.pi/18,8*np.pi/18]
    h1 = np.ones((Nt, 1)) - 1j * np.zeros((Nt, 1))
    for j in range(N_users - 1):
        h = np.ones((Nt, 1)) - 1j * np.zeros((Nt, 1))
        for i in range(Nt):
            h[i, 0] =np.cos((i)*thetas[j]) - 1j  * np.sin((i)*thetas[j])
        h1 = np.concatenate((h1, h), axis=1)
    return h1
...
if __name__ == "__main__":
    ...
    SR,t=order_g(H_estimate,H_reals,groups,weight, tolerance, Pt, N_users, Nt, 0, alphas)
    print(SR)
    T2=time.time()
